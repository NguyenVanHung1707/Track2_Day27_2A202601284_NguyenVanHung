...
def test_sustained_fast_burn_pages_oncall():
    result = multiwindow_burn(short_window_burn=14.5, long_window_burn=14.2)
    assert result["page"] is True
    assert result["severity"] == "critical"


def test_transient_spike_does_not_page():
    # Short window high, but long window low -> transient spike, no page
    result = multiwindow_burn(short_window_burn=8.0, long_window_burn=1.2)
